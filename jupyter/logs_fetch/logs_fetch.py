...
def fetch_logs_and_save_both():
    fetched_list = get_cloud_function_logs(CloudFunction.OPTIMIZED, PROJECT_ID, 100)
    csv_log_saver.save_to_csv(CloudFunction.OPTIMIZED.value, fetched_list)

    fetched_list = get_cloud_function_logs(CloudFunction.BASELINE, PROJECT_ID, 100)
